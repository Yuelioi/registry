...
├── admin.js      ← 可选：浏览器端 admin 脚本
├── public.js     ← 可选：浏览器端前台脚本
└── README.md     ← 建议提供
```
...
| `features` | string[] | | 插件使用的特性列表，用于市场筛选。见下方 |
...
在 `features` 数组中列出插件使用的特性，市场可据此筛选：
...
扩展字段均为可选，缺失时留空或空数组。建议在更新版本时顺便补全 `capabilities` 和 `features` 字段。
